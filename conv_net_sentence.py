...
def train_conv_net(datasets,  # datasets == pos/neg data folds
                   w2v_vectors,
                   img_w=300,
                   filter_hs=[3,4,5],
                   hidden_units=[100,2],
    ...
    index = T.lscalar()
    x = T.matrix('x')  # returns a matrix with the name 'x'
    y = T.ivector('y')
    Words = theano.shared(value = w2v_vectors, name ="Words")
    zero_vec_tensor = T.vector()
    zero_vec = np.zeros(img_w)
    # set_zero => replace shared variable Words (aka w2v_vectors) with null vectors
...
if __name__=="__main__":
    ...
    x = cPickle.load(open(filename,"rb"))
